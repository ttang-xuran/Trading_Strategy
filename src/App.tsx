...
function App() {
  ...
  const loadDataSources = async () => {
    try {
      // Use static data by default, with API as enhancement
      ...
      // Try API in background for future requests
      try {
        const isApiAvailable = await apiService.isAvailable()
        if (isApiAvailable) {
          console.log('API is available, will use for future requests')
          // Don't switch to API immediately to avoid delay
