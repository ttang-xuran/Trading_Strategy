#!/usr/bin/env python3
"""
Run optimized strategy (25, 0.4, 2.0) on Binance data with yearly analysis
"""
...
def run_optimized_binance_backtest():
    """Run optimized strategy on Binance data with yearly breakdown"""
    
    print("=== OPTIMIZED STRATEGY ON BINANCE DATA ===")
    print("Parameters: lookback_period = 25, range_mult = 0.4, stop_loss_mult = 2.0")
    print("Data Source: Binance BTC/USDT")
    ...
    # Summary statistics
    print("\n" + "="*70)
    print("BINANCE SUMMARY STATISTICS:")
    ...
    # Save Binance optimized results
    if strategy.trades:
        binance_optimized_trades_df = pd.DataFrame(strategy.trades)
        binance_optimized_trades_df.to_csv('/home/ttang/Super BTC trading Strategy/binance_optimized_trades.csv', index=False)
        print(f"\nBinance optimized trades saved to: binance_optimized_trades.csv")
    
    return yearly_results, strategy

def compare_coinbase_vs_binance_optimized():
    """Compare optimized strategy results between Coinbase and Binance"""
    
    print(f"\n{'='*80}")
    print("COINBASE VS BINANCE COMPARISON (OPTIMIZED PARAMETERS)")
    print(f"{'='*80}")
    
    # Load Coinbase optimized results
    try:
        ...
        print(f"COINBASE (Full Period 2015-2025):")
        print(f"  Return: {coinbase_return:,.1f}%")
        print(f"  Trades: {coinbase_trade_count}")
        print(f"  Win Rate: {coinbase_win_rate:.1f}%")
        print(f"  Final Equity: ${coinbase_final_equity:,.0f}")
        
    except Exception as e:
        print(f"Could not load Coinbase results: {e}")
        return
    
    # Load Binance optimized results
    try:
        ...
        print(f"\nBINANCE (Period 2017-2025):")
        print(f"  Return: {binance_return:,.1f}%")
        print(f"  Trades: {binance_trade_count}")
        print(f"  Win Rate: {binance_win_rate:.1f}%")
        print(f"  Final Equity: ${binance_final_equity:,.0f}")
        
        # Calculate differences (noting different time periods)
        print(f"\nDIFFERENCE (Binance vs Coinbase):")
        print(f"  Return Difference: {binance_return - coinbase_return:+,.1f}%")
        print(f"  Trade Difference: {binance_trade_count - coinbase_trade_count:+d}")
        print(f"  Win Rate Difference: {binance_win_rate - coinbase_win_rate:+.1f}%")
        print(f"  Equity Difference: ${binance_final_equity - coinbase_final_equity:+,.0f}")
        
        print(f"\nNOTE: Binance data starts from 2017-08-17, missing the profitable 2015-2017 period")
        
    except Exception as e:
        print(f"Could not load Binance results: {e}")

def show_binance_detailed_trades(strategy):
    """Show detailed Binance trades by year"""
    
    print(f"\n{'='*80}")
    print("DETAILED BINANCE TRADES BY YEAR (OPTIMIZED)")
    print(f"{'='*80}")
...
def main():
    ...
    # Run optimized strategy on Binance data
    yearly_results, strategy = run_optimized_binance_backtest()
    
    # Compare with Coinbase results
    compare_coinbase_vs_binance_optimized()
    
    # Show detailed trades
    if yearly_results:
        show_binance_detailed_trades(strategy)
